#!/usr/bin/env python3
# Name: Avani Narayan
# Group Members: Aadhithya Somasundaram
...
def main(inFile = None, options = None):
    ...
    for head, seq, qual in myReader.readFastq():
        ...
        if input == "P33":
            ...
            if output == "P33":
                qual, seq = P64B_to_P33(qual, seq)
                if qual == "ERROR":
                    continue
            else:
                qual, seq = P64B_to_P64(qual, seq)
                if qual == "ERROR":
                    continue
